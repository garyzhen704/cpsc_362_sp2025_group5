...
# Bullet constants
BULLET_SPEED = 400  # Speed of the bullet (pixels per second)
BULLET_LIFETIME = .75  # Lifetime of the bullet (seconds)

class Bullet:
    def __init__(self, pos: Vector, direction: Vector, color):
        self.position = pos
        self.velocity = direction.normalized() * BULLET_SPEED
        self.color = color
    ...
    def update(self, fps: float):
      ...
      for obj in list(globals.game_objects):
          ...
          if isinstance(obj, Asteroid) and self.hitbox.colliderect(obj.rect):
              ...
              globals.delete_obj(self)  # Remove bullet
              if self in globals.player.bullets:  # If bullet is still in player's list
                  globals.player.bullets.remove(self)
              return  # Bullet is gone, skip the rest

      # Destroy bullet if its lifetime exceeds limit
      if self.age > self.lifetime:
          globals.delete_obj(self)  # Delete bullet when its lifetime exceeds
          if self in globals.player.bullets:  # If bullet is still in player's list
              globals.player.bullets.remove(self)
